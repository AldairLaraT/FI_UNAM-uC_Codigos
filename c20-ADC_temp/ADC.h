 ...
 * 
 * Código:      Archivo de cabecera del ADC
 * 
...
#define ADC1_SS2_Initiate()         (ADC1_PSSI_R |= ADC_PSSI_SS2)                   /*  ADC0 => SS2 Initiate -> Begin sampling on SS2 */

    /*  Lectura del estado del ADC */
#define ADC1_SS2_RawInterrupt_wait()    while (!(ADC1_RIS_R & ADC_RIS_INR2)) {}     /*  ADC1 => INR2: SS2 Raw Interrupt Status -> A sample has completed conversion */

    /*  Lectura del resultado de conversión */
#define ADC1_SS2_FIFOread()         (ADC1_SSFIFO2_R & ADC_SSFIFO2_DATA_M)           /*  ADC1 SS2 => DATA: Conversion Result Data */

    /*  Limpieza de las banderas INn (ADCISC) y INRn (ADCRIS) */
#define ADC1_SS2_ClearFlags()       (ADC1_ISC_R |= ADC_ISC_IN2)                     /*  ADC1 => IN2: SS2 Interrupt Status and Clear -> IN2 bit (ADCISC) and INR2 bit (ADCRIS) cleared */
