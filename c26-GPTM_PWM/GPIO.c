...
void GPIO_PortM0_Init_T2CCP0(void) {
    ...
    /*  Paso 1: Habilitar la señal de reloj del GPIO (RCGCGPIO) y esperar a que se estabilice (PRGPIO) */
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R11;                                       /*  R11: GPIO PortM Run Mode Clock Gating Control -> Enabled */
    while (!(SYSCTL_PRGPIO_R & SYSCTL_PRGPIO_R11)) {}                               /*  R11: GPIO PortM Peripheral Ready -> GPIO PortN is ready for access? */

    /*  Paso 2: Configurar la dirección del GPIO (GPIODIR) */
    GPIO_PORTM_DIR_R |= GPIO_PIN_0;                                                 /*  PortM[0] => DIR: Data direction -> Output */

    /*  Paso 3: Habilitar la función alterna del GPIO (GPIOAFSEL) */
    GPIO_PORTM_AFSEL_R |= GPIO_PIN_0;                                               /*  PortM[0] => AFSEL: Alternate Function Select -> Alternate hardware function */
    ...
    /*  Paso 9: Habilitar las funciones digitales del GPIO (GPIODEN) */
    GPIO_PORTM_DEN_R |= GPIO_PIN_0;                                                 /*  PortM[0] => DEN: Digital Enable -> Enabled */
